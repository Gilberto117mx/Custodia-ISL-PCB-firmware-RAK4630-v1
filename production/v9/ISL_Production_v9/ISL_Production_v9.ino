/*
 * ============================================================================
 * ISL Board (RAK_feather) - PRODUCTION firmware v9
 * ============================================================================
...
// BLE offload pacing. RUI3 api.ble.uart has no "connected" callback, so we
// advertise, wait BLE_SETTLE_MS for connect+subscribe, then run the handshake.
constexpr uint32_t BLE_SETTLE_MS       = 5000;  // (unused in the one-directional path)
constexpr uint32_t BLE_SUBSCRIBE_TIMEOUT_MS = 20000; // wait for the drone to connect + enable notify
constexpr uint32_t BLE_LINE_GAP_MS     = 8;     // per-frame pacing during the bulk blast
constexpr uint32_t BLE_HOLD_MS         = 300;   // flush before waiting on the drone's reply
constexpr uint32_t BLE_GO_TIMEOUT_MS   = 8000;  // wait for the drone's "GO" before streaming
constexpr uint32_t BLE_ACK_TIMEOUT_MS  = 12000; // wait for the drone's "ACK" after "END"
...
// ============================================================================
// BLE drone offload - custom-open NUS, ONE-DIRECTIONAL framed blast.
//   PROVEN (custom-open POC): api.ble.custom + RAK_SET_OPEN gives NO-PAIRING
//   notifications (no "Pairing procedure fail" hang) and advertises the NUS
//   service UUID, so the receiver discovers it by SERVICE. We blast the whole
//   ring one-directionally (no GO/ACK -> nothing can hang), framed into fixed
//   20 B notifications ([len][payload]). The ring is NOT cleared: the receiver
//   dedups by record seq, and the ring self-bounds by rolling off the oldest, so
//   a missed pass loses nothing while ACCEL_RING_RECORDS covers > one interval.
// ============================================================================
#define BLE_FRAME_LEN 20

// NUS 128-bit base UUID, MSB-first (bytes[2..3] = the 16-bit field). Chars 0x0002
// (RX/write) and 0x0003 (TX/notify) inherit this base -> 6E400002 / 6E400003.
static uint8_t nus_base[16] = {
    0x6E, 0x40, 0x00, 0x01, 0xB5, 0xA3, 0xF3, 0x93,
    0xE0, 0xA9, 0xE5, 0x0E, 0x24, 0xDC, 0xCA, 0x9E };
RAKBleCharacteristic bleTxc = RAKBleCharacteristic(0x0003);   // NOTIFY (tracker -> drone)
RAKBleCharacteristic bleRxc = RAKBleCharacteristic(0x0002);   // WRITE  (unused; present for NUS discovery)
static volatile bool bleSubscribed = false;

void ble_cccd_cb(uint16_t uuid, uint8_t *cccd) { (void)uuid; (void)cccd; bleSubscribed = bleTxc.notifyEnabled(); }

static void bleCustomStart()
{
    api.ble.custom.init();
    RAKBleService nus = RAKBleService(nus_base);
    nus.begin();
    bleTxc.setProperties(RAK_CHR_PROPS_NOTIFY);
    bleTxc.setPermission(RAK_SET_OPEN);          // OPEN: no pairing/encryption
    bleTxc.setFixedLen(BLE_FRAME_LEN);
    bleTxc.setCccdWriteCallback(ble_cccd_cb);
    bleTxc.begin();
    bleRxc.setProperties(RAK_CHR_PROPS_WRITE);
    bleRxc.setPermission(RAK_SET_OPEN);
    bleRxc.setFixedLen(BLE_FRAME_LEN);
    bleRxc.begin();
...
// One fixed 20 B notify: frame[0]=valid length, frame[1..len]=payload, rest 0.
static void bleNotifyFrame(const uint8_t *data, uint8_t len)
{
    uint8_t f[BLE_FRAME_LEN];
    if (len > BLE_FRAME_LEN - 1) len = BLE_FRAME_LEN - 1;
    f[0] = len;
    memcpy(f + 1, data, len);
    if ((int)len + 1 < BLE_FRAME_LEN) memset(f + 1 + len, 0, BLE_FRAME_LEN - 1 - len);
    bleTxc.notify(f);
    delay(BLE_LINE_GAP_MS);                      // pace so the link buffer keeps up
}
static void bleSendBytes(const uint8_t *buf, int n)
{
    int off = 0;
    while (off < n) {
        int chunk = (n - off > BLE_FRAME_LEN - 1) ? (BLE_FRAME_LEN - 1) : (n - off);
        bleNotifyFrame(buf + off, (uint8_t)chunk);
        off += chunk;
    }
}

// Blast the whole ring one-directionally. Returns true if a subscriber attached
// (so we streamed); false if nobody connected. Ring is NOT cleared either way.
bool bleDroneOffload()
{
    ...
    bleSubscribed = false;
    bleCustomStart();
    DBG("[BLE] custom-open NUS up (no pairing), advertising, %u records. Waiting for subscribe...\r\n", nRecs);

    uint32_t t = millis();
    while (!bleSubscribed && (millis() - t) < BLE_SUBSCRIBE_TIMEOUT_MS) delay(20);
    if (!bleSubscribed) { DBG("[BLE] no subscriber - keeping data for next pass\r\n"); return false; }
    delay(300);                                  // let the CCCD settle before streaming
    ...
    int n = snprintf(line, sizeof(line), "E %u\n", nRecs);
    bleSendBytes((uint8_t *)line, n);
    delay(BLE_HOLD_MS);
    DBG("[BLE] blasted %u records (one-directional, no pairing). Ring kept; drone dedups.\r\n", nRecs);
    return true;
}
#else
void accelRingLoad() {}
...
{
    say("========================================================");
    say("ISL Board - PRODUCTION v9  (v8 + drone-pass bulk accel offload; 34 uA floor)");
    say("========================================================");
...
void setup()
{
    ...
    delay(300);
    say("ISL Production v9 boot. Heartbeat, then init...");
}
...
void loop()
{
    ...
    if (isDronePass()) {
        ...
        bool sent = bleDroneOffload();             // one-directional blast (no pairing, cannot hang)

        // The ring is NOT cleared: the drone dedups by seq and the ring rolls off
        // the oldest when full, so nothing is lost while capacity > one interval.
        DBG("== DRONE PASS done (%s) -> reboot-to-sleep ==\r\n", sent ? "blasted" : "no subscriber");
