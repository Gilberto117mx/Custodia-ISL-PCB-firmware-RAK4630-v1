/*
 * ============================================================================
 * ISL Board (RAK_feather) - PRODUCTION firmware v2  [MIN/SEC config]
 * ============================================================================
 *
 * Same firmware as production v1 (validated: ~3.6 h floor 155 uA @ 3.6 V; first
 * GNSS fix ~21 s; LoRa TX + ACK receiver-confirmed), with knobs in human units
 * (MINUTES / SECONDS, auto-converted to ms). The rest works in ms internally.
 *
 * CHANGE v1 -> v2: the battery reader is now CALIBRATED (test #2 final).
 *   v1 used analogReference(AR_INTERNAL) assuming 2.4 V full scale and
 *   read the battery ~30% low with a cliff near LDO dropout. On THIS RUI3 core
 *   AR_INTERNAL is ~3.67 V FS; direct-register SAADC returns 0. v2 does a one-time
 *   SAADC offset calibration to condition the ref, reads analogRead(AR_INTERNAL),
 *   and applies a through-origin cal:  Vbat_mV = raw * 1795 / 1000  (<=25 mV,
 *   3.2-3.7 V). See tests/ISL_Battery_ADC/README.md.  The packet's vbat field is
 *   now trustworthy - everything else is unchanged.
 *
 * Operate loop:
 *   wake -> read battery (quiet) -> GNSS fix (retries) -> build packet
 *        -> BUFFER -> if full/pending, LoRa TX + wait ACK (retries -> UNDELIVERED)
 *        -> deep-sleep GNSS_PERIOD -> repeat.
 *
 * ISL hardware layer (see README / docs/ISL_DeepSleep_Notes.md):
 *   Debug  = native USB `Serial` (drops in deep sleep; prints if(Serial)-guarded).
 *   GPS    = Serial0/UART1 (P0.19/20), EN=P1.02 active-low; teardown drives both
 *            UART pins LOW to isolate the externally-batteried module.
 *   RTC    = RV-3028 periodic timer -> ~INT on P0.21 (deep-sleep wake).
 *   Sleep  = api.ble.stop + FPU off + clearFPU + api.system.sleep.all.
 *   Battery= analogRead(AR_INTERNAL) + one-time SAADC offset cal + raw*1795/1000.
 *   Struct = alive-first (heartbeat, then init in loop()).
 * ============================================================================
 */
...
#include <stddef.h>
#include <Arduino.h>
#include <nrf.h>       // direct SAADC offset calibration (battery reader conditioning)
#include <Wire.h>
#include <TinyGPSPlus.h>
#include "Melopero_RV3028.h"

// ============================================================================
// USER CONFIGURATION  -->  set these in MINUTES / SECONDS / counts
// ============================================================================
// Long durations  -> MINUTES.   Short / turnaround durations -> SECONDS.
// Plain counts (buffer size, retry counts, device id) are just numbers.
// The *_MS values the code uses are derived automatically further down - you do
// NOT edit those.
constexpr uint32_t GNSS_PERIOD_MIN       = 15;    // idle deep-sleep between cycles (deployment target ~120)
...
constexpr uint8_t  MAX_TX_RETRIES        = 3;    // TX attempts before a packet is archived UNDELIVERED
constexpr uint32_t GNSS_FIX_TIMEOUT_SEC  = 90;   // max listen time per fix attempt
...
// ---------------------------------------------------------------------------
// UNIT CONVERSION (do not edit) - everything below here works in milliseconds.
// ---------------------------------------------------------------------------
constexpr uint32_t SECONDS = 1000UL;             // ms per second
constexpr uint32_t MINUTES = 60UL * SECONDS;     // ms per minute
...
// Fixed hardware settle times (not user knobs) - kept in ms.
#define GPS_BOOT_DELAY_MS   500UL     // power-up settle after EN on (tests #3/#6)
#define GPS_POWERDOWN_MS    250UL     // let the TX line go quiet after cutting EN
#define WARMUP_MS           3000UL    // alive-first heartbeat before init
constexpr uint32_t SLEEP_BACKSTOP_MS = 60UL * MINUTES;  // 1 h safety (RTC wakes us far sooner)

// Battery ADC - CALIBRATED (test #2 final). analogRead(AR_INTERNAL) is ~3.67 V FS
// on this core; a through-origin single constant absorbs FS + divider + gain.
#define ADC_SAMPLES         31
#define VBAT_CAL_NUM        1795UL     // Vbat_mV = raw * 1795 / 1000  (<=25 mV, 3.2-3.7 V)
...
// (~68 min); a 2 h cadence needs the 1/60 Hz tick mode.
...
// Unix seconds, integer math only (H. Hinnant civil-calendar algorithm).
uint32_t rtcNowUnix()
{
    int Y = rtc.getYear();
    int M = rtc.getMonth();
...
// ============================================================================
// DEEP SLEEP (rules: docs/ISL_DeepSleep_Notes.md; floor 155 uA @ 3.6 V)
// deepSleep() takes SECONDS - convert the *_MS knobs at the call site with /SECONDS.
// ============================================================================
...
// One-time SAADC offset calibration conditions the internal ref; then
// analogRead(AR_INTERNAL) median-of-31 + through-origin cal raw*1795/1000.
// ============================================================================
...
// Call ONCE at init (before the first readVbat_mV). Conditions the internal
// reference so analogRead reads it linearly (no LDO-dropout cliff), then hands
// the SAADC back to the core's analogRead driver.
void batteryAdcInit()
{
...
uint16_t readVbat_mV()
{
    ...
    uint16_t raw = v[m/2];
    return (uint16_t)(((uint32_t)raw * VBAT_CAL_NUM) / VBAT_CAL_DEN);   // Vbat_mV = raw*1795/1000
}
...
void gpsPowerOff()
{
    // Order matters (docs/ISL_DeepSleep_Notes.md rules 3+4):
...
{
    say("========================================================");
    say("ISL Board - PRODUCTION v2  (calibrated battery, min/sec config)");
    say("========================================================");
    ...
    // Config echo in the human units you set them in.
    DBG("[CFG] id=%03u  period=%lu min  fix_timeout=%lu s  retry_wait=%lu min  "
        "tx_retry=%lu min  ack=%lu s  settle=%lu s  buffer=%u  gnss_retries=%u  "
        "tx_retries=%u  WUR_wake=%d\r\n",
        (unsigned)DEVICE_ID,
        ...
        TRACKER_BUFFER_SIZE, MAX_GNSS_RETRIES, MAX_TX_RETRIES, ENABLE_WUR_WAKE);
}
...
void setup()
{
    ...
    delay(300);
    say("ISL Production v2 (calibrated battery) boot. Heartbeat, then init...");
}
