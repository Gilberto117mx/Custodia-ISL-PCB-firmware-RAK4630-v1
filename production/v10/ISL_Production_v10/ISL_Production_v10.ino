/*
 * ============================================================================
 * ISL Board (RAK_feather) - PRODUCTION firmware v10  (COLLAR)
 * ============================================================================
 * v10 = v9 + the FINAL collar-side BLE offload, using the EXACT transport that ran
 * 5 clean cycles in v8: RUI3 `api.ble.uart` (Nordic UART Service), FIRE-AND-FORGET,
 * ONE-DIRECTIONAL, ID-tagged. The drone/receiver is another engineer's board
 * (different code) - our job is only that the collar EMITS its accelerometer ring
 * so any standard BLE central can receive it, and that the collar can NEVER freeze
 * doing so.
 *
 * Why revert to v8's transport: v9/early-v10 used `api.ble.custom` + notify(). A
 * notify() on a link the drone had just dropped BLOCKS FOREVER inside the RUI3
 * stack (the CPU is stuck in vendor code, so an application watchdog can't even
 * fire) - that was the "collar frozen in the BLE stage" bug. `api.ble.uart.write()`
 * has no such property: it pushes into the NUS TX buffer and returns whether or not
 * a central is listening. The whole offload is a bounded sum of delay()s, so the
 * collar always leaves the BLE stage on time. This is the "chrono".
 *
 * The offload (fire-and-forget):
 *  - advertise by NAME "Custodia-Tracker" (api.ble.uart advertises the name, not
 *    the NUS UUID -> the receiver matches by name);
 *  - wait a FIXED BLE_SETTLE_MS for a central to connect + subscribe;
 *  - api.ble.uart.write() each ID-tagged text line, paced BLE_LINE_GAP_MS apart:
 *        "ID <dev> recs=<N>"                          (opening announce)
 *        "R <dev> <seq> <ts> <n> <sum>" + n "x,y,z"   (per record)
 *        "E <dev> <N>"                                (end)
 *  - hold BLE_HOLD_MS, then reboot-to-sleep (which is what clears BLE; we never
 *    api.ble.stop() - a stop while connected re-advertises and the reconnect can
 *    wedge the next sleep).
 *  - No ACK (one-directional), so OFFLOAD_DELETE_AFTER_SEND defaults to 0: KEEP the
 *    ring + let the drone dedup by seq (no data lost when no drone was there). Set 1
 *    to clear after every blast (assumes delivery; loss acceptable).
 *
 * NOTE: connection stability for a long blast is tuned on the CENTRAL (drone)
...
//   collected every ACCEL_EVERY_N_FIXES-th fix and appended to a flash ring; the
//   whole ring is offloaded over BLE only on a drone pass (WUR / SIMULATE_WUR).
// ----------------------------------------------------------------------------
...
constexpr uint16_t ACCEL_EVERY_N_FIXES = 1;   // collect a record every Nth fix (1 = every fix)
...
// flash is ~132 KB usable (MEASURED, tests/ISL_Flash_Probe) -> up to ~215 records
// @10 Hz, so this fits a full 2-week gap with margin. 16 is a bench size; raise to
// 56-64 for a real deployment.
constexpr uint16_t ACCEL_RING_RECORDS  = 16;
...
// BLE offload pacing -- v10 uses the SAME transport that ran 5 clean cycles in
// v8: RUI3 `api.ble.uart` (Nordic UART Service), FIRE-AND-FORGET. We advertise by
// NAME, wait a FIXED BLE_SETTLE_MS for a central to connect + subscribe, then just
// `api.ble.uart.write()` each line paced BLE_LINE_GAP_MS apart, hold, and reboot-
// to-sleep (which is what clears BLE). There is NO subscribe callback, NO
// notify()-on-a-live-link, and NO drone->collar handshake -- so there is nothing
// that can block on a dead link. This is the "chrono": the whole stage is a fixed
// sum of delays, bounded regardless of what the receiver does, so the collar can
// NEVER freeze in BLE.
//
// v10: after a blast we cannot know if the drone actually got it (one-directional,
// no ACK). Default is therefore to KEEP the ring and let the drone dedup by seq
// (never loses data; ring self-bounds by rolling off the oldest). Set to 1 to
// CLEAR the ring after every blast (assumes delivery; data loss acceptable).
#define OFFLOAD_DELETE_AFTER_SEND  0

constexpr uint32_t BLE_SETTLE_MS       = 5000;  // fixed wait for the central to connect+subscribe before streaming
constexpr uint32_t BLE_LINE_GAP_MS     = 20;    // per-line pacing so the NUS TX buffer keeps up
constexpr uint32_t BLE_HOLD_MS         = 500;   // let the last notification flush before we reboot
constexpr uint32_t BLE_STREAM_MAX_MS   = 60000; // safety cap on the whole blast (pure delay-bounded; can't hang)
...
#define FLASH_OFF_ACCEL_META  0x0400   // ring metadata (magic, head, count, nextSeq, lastDumpUnix)
#define FLASH_OFF_ACCEL_DATA  0x0500   // record i at DATA + i * sizeof(AccelRecord)
#define ACCEL_META_MAGIC      0xACCE1D09UL
struct AccelRecord {
    uint32_t seq;
...
// ============================================================================
// BLE drone offload - EXACT v8 transport (proven, 5 clean cycles). RUI3
// `api.ble.uart` (Nordic UART Service), FIRE-AND-FORGET:
//   1) advertise by NAME (RUI3 api.ble.uart advertises the name, not the service
//      UUID -> the receiver matches by name "Custodia-Tracker");
...
// There is NO connect/subscribe callback, NO notify()-on-a-live-link, and NO
// drone->collar handshake -- `api.ble.uart.write()` just pushes into the NUS TX
// buffer and returns whether or not a central is listening. The whole stage is a
// bounded sum of delays, so the collar can NEVER freeze here (this is the fix for
// the api.ble.custom notify() that blocked forever on a dropped link). We do NOT
// call api.ble.stop(): a stop while connected re-advertises and the reconnect can
// wedge the next sleep; the reboot-to-sleep clears BLE deterministically instead.
//
// Payload is the same ID-tagged text stream as before so the receiver can dedup by
// seq and verify per-record checksums; it is just carried over the NUS byte stream
// (newline-delimited) rather than fixed 20 B frames.
// ============================================================================
...
bool bleDroneOffload()
{
    ...
    DBG("[BLE] api.ble.uart advertising '%s', %u records, settling %lu ms (fire-and-forget)...\r\n",
        nm, nRecs, (unsigned long)BLE_SETTLE_MS);
    ...
    char line[48];
    // opening announce: who I am + how many records
    int n = snprintf(line, sizeof(line), "ID %03u recs=%u\n", (unsigned)DEVICE_ID, nRecs);
    bleWriteLine(line, n);
    ...
    for (uint16_t r = 0; r < nRecs; r++) {
        ...
        if ((int32_t)(millis() - streamStart) > (int32_t)BLE_STREAM_MAX_MS) {
            DBG("[BLE] stream cap hit after %u records\r\n", r);
            break;
        }
        if (!accelRingGet(r)) continue;
        uint32_t rsum = 0;                        // per-record checksum for the drone to verify
        for (uint16_t i = 0; i < accelRec.count; i++)
            rsum += (uint16_t)accelRec.xyz[i][0] + (uint16_t)accelRec.xyz[i][1] + (uint16_t)accelRec.xyz[i][2];
        n = snprintf(line, sizeof(line), "R %03u %lu %lu %u %lu\n", (unsigned)DEVICE_ID,
                     (unsigned long)accelRec.seq, (unsigned long)accelRec.ts, accelRec.count, (unsigned long)rsum);
        bleWriteLine(line, n);
        for (uint16_t i = 0; i < accelRec.count; i++) {
    ...
    n = snprintf(line, sizeof(line), "E %03u %u\n", (unsigned)DEVICE_ID, nRecs);
    bleWriteLine(line, n);
    delay(BLE_HOLD_MS);                          // let the last notification flush

    DBG("[BLE] blasted %u records as id=%03u (fire-and-forget) - reboot-to-sleep clears BLE\r\n",
        nRecs, (unsigned)DEVICE_ID);
...
void doCollect()
{
    ...
    // v9: on a real fix, every ACCEL_EVERY_N_FIXES-th fix, collect a 10 s record
    // and append it to the flash ring (offloaded later on a drone pass). BLE is
    // NOT used here - only LoRa (the position packet) transmits this cycle.
    if (r.haveFix) {
        fixCounter++;
        if (ACCEL_EVERY_N_FIXES <= 1 || (fixCounter % ACCEL_EVERY_N_FIXES) == 0) {
            DBG("== ACCEL collect %lu s -> ring (fix #%lu) ==\r\n",
                (unsigned long)ACCEL_SECS, (unsigned long)fixCounter);
            collectAccelToRing(r.haveFix ? r.fixUnix : rtcNowUnix());
        }
    }
...
{
    say("========================================================");
    say("ISL Board - PRODUCTION v10 (COLLAR: api.ble.uart fire-and-forget accel offload)");
    say("========================================================");
...
void setup()
{
    ...
    delay(300);
    say("ISL Production v10 boot. Heartbeat, then init...");
}
...
void loop()
{
    ...
    if (isDronePass()) {
        ...
        bool sent = bleDroneOffload();             // v8 fire-and-forget blast (cannot hang)

        // v10: fire-and-forget has no ACK, so `sent` is always true (a blast was
        // attempted). Default OFFLOAD_DELETE_AFTER_SEND=0 KEEPS the ring so the drone
        // dedups by seq and no data is lost when no drone was actually there. Set to
        // 1 to CLEAR after every blast (assumes delivery; data loss acceptable).
        if (OFFLOAD_DELETE_AFTER_SEND && sent) accelRingClear();
        DBG("== DRONE PASS done (%s) -> reboot-to-sleep ==\r\n",
            OFFLOAD_DELETE_AFTER_SEND ? "blasted+cleared" : "blasted, kept for dedup");
