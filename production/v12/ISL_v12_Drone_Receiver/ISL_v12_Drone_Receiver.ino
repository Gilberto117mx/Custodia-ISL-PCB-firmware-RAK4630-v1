/*
 * ============================================================================
 * ISL v12 Drone Receiver  ·  REFERENCE receiver for the v12 collar
 * ============================================================================
 ...
 *
 * Wire format & transport are IDENTICAL to v11 (proven). The only difference is what
 * the collar SENDS: v12 transmits ONLY NEW accel data each pass (clear-after-send),
 * so across passes you should see all-NEW seqs -- never a cross-pass `dup`. The only
 * `dup` you'll see is within a single pass, from the 2nd BLE_BLAST_REPEATS blast
 * (that in-pass redundancy is why we still dedup here).
 *
...
void setup() {
  ...
  Serial.println("=== ISL v12 Drone Receiver (reference) - api.ble.uart NUS, name-match, NEW data each pass ===");
