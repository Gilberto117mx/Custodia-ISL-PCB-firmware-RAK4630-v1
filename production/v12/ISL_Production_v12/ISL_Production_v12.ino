/*
 * ============================================================================
 * ISL Board (RAK_feather) - PRODUCTION firmware v12  (COLLAR)
 * ============================================================================
 * v12 = v11 (the LAST SUCCESSFUL / bench-verified build) with ONE behavioural
 * change: every drone pass transmits ONLY NEW accelerometer data.
 *
 * v11 (proven: 3 passes, 20 records, 0 bad, collar never froze) KEEPS the accel
 * ring after each blast and re-sends the WHOLE ring every pass, letting the drone
 * dedup by seq. v12 instead CLEARS the ring after each blast
 * (OFFLOAD_DELETE_AFTER_SEND = 1), so a pass carries only the records collected
 * since the previous pass -- a fresh, non-overlapping batch every time, never a
 * re-send.
 *
 * Trade-off (intentional): fire-and-forget has no ACK, so a missed pass loses those
 * records permanently. That on-collar buffering/replay is DELIBERATELY dropped here
 * -- the other engineer owns retention with a different BLE approach. Our job in v12
 * is only "each transmission is new data." Set OFFLOAD_DELETE_AFTER_SEND back to 0 to
 * get v11's keep-and-dedup behaviour.
 *
 * EVERYTHING ELSE IS IDENTICAL TO v11 -- same proven, no-freeze transport and wire
 * format:
 *  - RUI3 `api.ble.uart`, FIRE-AND-FORGET (the bounded sum of delay()s is the
 *    "chrono"; the collar can never freeze in BLE);
 *  - advertise by NAME "Custodia-Tracker"; fixed BLE_SETTLE_MS wait; write each line
 *    paced BLE_LINE_GAP_MS; hold BLE_HOLD_MS; reboot-to-sleep clears BLE (never
 *    api.ble.stop());
 *  - every line fits in ONE ~20 B NUS notification so a dropped packet can't corrupt
 *    its neighbour; blasted BLE_BLAST_REPEATS times for loss resilience:
 ...
 * unencrypted and validate fine (proven in the v11 run).
 * ----------------------------------------------------------------------------
...
{
    say("========================================================");
    say("ISL Board - PRODUCTION v12 (COLLAR: api.ble.uart fire-and-forget accel offload, NEW data each pass)");
    say("========================================================");
...
void setup()
{
    ...
    delay(300);
    say("ISL Production v12 boot. Heartbeat, then init...");
}
