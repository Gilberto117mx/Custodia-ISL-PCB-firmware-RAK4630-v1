/*
 * ============================================================================
 * ISL Board (RAK_feather) - PRODUCTION firmware v11  (COLLAR)
 * ============================================================================
 * v11 = v10 (kept in the repo as reference) with the BLE offload DATA-INTEGRITY
 * fix. v10's transport was already correct in the one way that mattered most - the
 * collar NEVER froze across a whole overnight-style run of GNSS/LoRa cycles and 6+
 * drone passes (RUI3 `api.ble.uart`, FIRE-AND-FORGET; the bounded sum of delay()s
 * is the "chrono"). But in the v10 run the receiver connected + subscribed yet
 * validated ZERO records.
 *
 * ROOT CAUSE (v10 run): api.ble.uart fragments each write into ~20-byte NUS
 * notifications with NO flow control, so on a lossy/unencrypted link a notification
 * can drop. v10's per-record header "R 051 42 1785165983 100 <sum>" is 30+ bytes =
 * two notifications; the first half (seq, ts) always arrived, the second half
 * (count, checksum) was routinely lost, so the receiver parsed a garbage sample
 * count (e.g. n=4294952064) and could never complete a record.
 *
 * v11 FIX - keep every line short enough to fit in ONE notification (<=20 B), so a
 * line arrives whole or is lost whole (that record just retries next pass); a
 * dropped packet can no longer corrupt its neighbour:
 *        "I <dev> <nrecs>"      opening announce (DEVICE_ID sent here, once)
 *        "R <seq> <n> <ck16>"   per-record header (16-bit checksum keeps it <=20 B)
 *        "<x>,<y>,<z>"          a sample
 *        "E <dev> <nrecs>"      end
 * ...and blast the whole ring BLE_BLAST_REPEATS times (default 2) so a single loss
 * gets another chance - the receiver dedups by seq, so repeats are free.
 *
 * Transport unchanged from v10 (still cannot freeze):
 *  - advertise by NAME "Custodia-Tracker"; fixed BLE_SETTLE_MS wait; write each line
 *    paced BLE_LINE_GAP_MS; hold BLE_HOLD_MS; reboot-to-sleep clears BLE (we never
 *    api.ble.stop()). No ACK -> OFFLOAD_DELETE_AFTER_SEND defaults to 0 (keep ring +
 *    drone dedups by seq; no data lost when no drone was there).
 *
 * NOTE: the drone/receiver is another engineer's board (different code). Connection
 * stability/encryption for a long blast is tuned on the CENTRAL side - out of scope
 * for the collar. The collar just advertises + fire-and-forget streams, and can
 * never freeze doing so. (See the v11 README for the "Pairing procedure fail" note:
 * on this RUI3 build the central-initiated pairing is cosmetic - notifications still
 * flow unencrypted - but the reference receiver now accepts Just-Works to try for a
 * stable encrypted link.)
 * ----------------------------------------------------------------------------
...
// cannot freeze.
//
// After a blast we cannot know if the drone actually got it (one-directional, no
// ACK), so default KEEPS the ring and lets the drone dedup by seq (never loses data;
// ring self-bounds by rolling off the oldest). Set to 1 to CLEAR after every blast.
...
constexpr uint32_t BLE_STREAM_MAX_MS   = 90000; // safety cap on the whole blast (pure delay-bounded; can't hang)
...
{
    say("========================================================");
    say("ISL Board - PRODUCTION v11 (COLLAR: api.ble.uart fire-and-forget accel offload, short-line + x2 blast)");
    say("========================================================");
...
void setup()
{
    ...
    delay(300);
    say("ISL Production v11 boot. Heartbeat, then init...");
}
