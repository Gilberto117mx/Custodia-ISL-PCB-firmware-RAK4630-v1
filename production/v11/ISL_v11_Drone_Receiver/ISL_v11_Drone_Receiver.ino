/*
 * ============================================================================
 * ISL v11 Drone Receiver  ·  REFERENCE receiver for the v11 collar
 * ============================================================================
 ...
 *
 * v11 keeps v10's proven transport (RUI3 `api.ble.uart`, fire-and-forget, NUS
 * advertised by NAME) and fixes the data-integrity problem the v10 run showed:
 * every collar line now fits in ONE ~20-byte NUS notification, so a dropped packet
 * loses a whole line instead of corrupting the next one, and the whole ring is sent
 * twice (BLE_BLAST_REPEATS) so we dedup and recover single losses.
 *
...
static uint32_t curDev=0, curSeq=0;
...
static void handleLine(char *ln)
{
  if (ln[0]=='I' && ln[1]==' ') {                        // "I <dev> <nrecs>"
    ...
    curSeq=seq; curN=nn; curSum=(uint16_t)ck; curGot=0; curCalc=0; curActive=true;
    curSkip = isSeen(seq);
...
void setup() {
  ...
  Serial.println("=== ISL v11 Drone Receiver (reference) - api.ble.uart NUS, name-match, dedup by seq ===");
