/*
 * ============================================================================
 * ISL Board (RAK_feather) - PRODUCTION firmware v5
 * ============================================================================
 * v5 = v4 + GPS RECEPTION DIAGNOSTIC. The v4 outdoor run seeded REAL UTC time
 * (~2026-07-14, hardened seed validated) but got 0 POSITION fixes in ~20x60 s
 * attempts (lat/lon=0, sats=0, PPS never lit). To tell 'no signal / antenna'
 * from 'weak / needs more sky' WITHOUT USB, v5 parses satellites-IN-VIEW from
 * the GSV sentences (GP/BD/GL/GA) and APPENDS the peak count to the packet as
 * a 7th field 'SV=N'. The receiver shows it in Raw, so you diagnose headless:
 *   SV=0 every packet  -> module tracks NO satellites (antenna/hardware/sky)
 *   SV>=4 but lat/lon=0 -> signal present, fix not resolving (more sky/time)
 * Also GNSS_FIX_TIMEOUT back to 90 s (the window that gave our only past fix).
 * On-air format is now 7 fields; the v2 receiver still ACKs (uses fields 0/1)
 * and prints the full raw line, so no receiver change is needed.
 * ----------------------------------------------------------------------------
 * v4 was = the HARDENED v3 (which an earlier bench run was NOT using - the
...
struct PacketSlot {
    ...
    uint8_t  hasFix;
    uint8_t  txAttempts;
    uint32_t satsInView;   // v5: peak satellites-in-view (GSV) this cycle - reception diag
};  // 24 bytes
...
bool gpsAcquire(uint32_t timeout_ms, int32_t *latE6, int32_t *lonE6, uint16_t *sats,
                uint32_t *ttff_ms, uint16_t *peakInView)
{
    TinyGPSPlus gps;
    // v5: satellites-IN-VIEW per constellation (GSV field 3 = total in view).
    ...
    *peakInView = 0;
    ...
    while ((millis() - start) < timeout_ms) {
        ...
        if (inview > *peakInView) *peakInView = inview;
...
bool tryOneFix(int32_t *latE6, int32_t *lonE6, uint16_t *sats, uint32_t *ttff_ms, uint16_t *peakInView)
{
    gpsPowerOn();
    bool ok = gpsAcquire(GNSS_FIX_TIMEOUT_MS, latE6, lonE6, sats, ttff_ms, peakInView);
    DBG("[GPS] %s in %lu ms  used=%u  IN-VIEW peak=%u  lat=%ld lon=%ld\r\n",
        ok ? "FIX" : "no fix",
        (unsigned long)*ttff_ms, *sats, *peakInView, (long)*latE6, (long)*lonE6);
    gpsPowerOff();
    return ok;
}
...
void doCollect()
{
    ...
    uint32_t ttff  = 0;
    bool     haveFix = false;
    uint16_t peakInView = 0, sessionPeak = 0;

    for (uint8_t attempt = 1; attempt <= MAX_GNSS_RETRIES; attempt++) {
        DBG("[GPS] attempt %u/%u\r\n", attempt, MAX_GNSS_RETRIES);
        bool got = tryOneFix(&latE6, &lonE6, &sats, &ttff, &peakInView);
        if (peakInView > sessionPeak) sessionPeak = peakInView;
        if (got) { haveFix = true; break; }
...
{
    say("========================================================");
    say("ISL Board - PRODUCTION v5  (GPS reception diag: SV in packet)");
    say("========================================================");
...
void setup()
{
    ...
    delay(300);
    say("ISL Production v5 boot. Heartbeat, then init...");
}
