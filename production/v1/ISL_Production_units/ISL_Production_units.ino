/*
 * ============================================================================
 * ISL Board (RAK_feather) - PRODUCTION firmware v1  [MIN/SEC config variant]
 * ============================================================================
 *
 * IDENTICAL firmware to ISL_Production (validated ~3.6 h run, floor 155 uA @
 * 3.6 V) - the ONLY difference is the USER CONFIGURATION block: durations are
 * set in human units (MINUTES for long ones, SECONDS for short/turnaround ones)
 * and converted to milliseconds automatically. The rest of the code is byte-for-
 * byte the same and still works in ms internally.
 *
 * Operate loop:
 ...
 *
 * ISL hardware layer (see ISL_Production README / docs/ISL_DeepSleep_Notes.md):
...
// ============================================================================
// BATTERY (AIN7/P0.31, 10k/10k) - median + reject, read while quiet.
// ============================================================================
...
uint16_t readVbat_mV()
{
    ...
    int med = valid[m/2];
    uint32_t vbat_mV = ((uint32_t)med * 4800UL) / 4095UL;  // med * 2.4 * 2.0 * 1000 / 4095
...
{
    say("========================================================");
    say("ISL Board - PRODUCTION v1  (min/sec config variant)");
    say("========================================================");
...
void setup()
{
    ...
    delay(300);
    say("ISL Production v1 (min/sec) boot. Heartbeat, then init...");
}
