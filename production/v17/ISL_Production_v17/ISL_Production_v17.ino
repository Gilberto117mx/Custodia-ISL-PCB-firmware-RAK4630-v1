/*
 * ============================================================================
 * ISL Board (RAK_feather) - PRODUCTION firmware v17  (COLLAR)
 * ============================================================================
 * v17 = v16, PORTED to ISL PCB iteration3 (the "onboard accelerometer" board).
 * Feature set is byte-for-byte v16 (durable 1-week LoRa backlog, GPS cell health,
 * cold-fix budget, two accel/BLE timers, no-freeze BLE offload). ONLY the hardware
 * interface changed, in two places the iteration3 schematic introduced:
 *   1) ACCELEROMETER: the external Grove LIS3DHTR on I2C (P0.24/P0.25) is replaced by
 *      the ONBOARD LIS3DHTR (U5) on a 4-wire SPI bus shared with the AS3933 WuR
 *      (CLK P0.03, MOSI P0.30, MISO P0.29, accel CS P0.28). Driver rewritten to
 *      bit-bang SPI (mode 3); same registers, same 100-sample records, same ring.
 *      Bonus: the Grove module's ~296 uA parasitic is gone -> lower sleep floor.
 *   2) GPS POWER: the L76K rail moved from an active-LOW P-FET to an active-HIGH
 *      TPS22918 load switch (L76K_EN=P1.02, HIGH=ON). Guarded by GPS_EN_ACTIVE_HIGH
 *      (=1 here); set it 0 if your physical board still has the P-FET.
 * RTC (P0.13/14 + INT P0.21), GPS UART (P0.19/20), and battery (P0.31/AIN7) are
 * IDENTICAL to iteration2, so everything else is untouched. Flash schema unchanged
 * (still 4) - a board swap keeps the same on-flash layout.
 * ----------------------------------------------------------------------------
 * v16 = v15 + DURABLE LONG-EXCURSION LoRa BACKLOG.
 *
 * Animals routinely roam OUT of repeater range for days. GPS keeps fixing the whole
 * time; the un-ACK'd real fixes are held in the flash-persisted pending[] backlog and
 * drained newest-first the instant the link returns. v15 kept only 5 slots (~10 h at a
 * 2 h cadence), so a multi-day excursion silently lost most of its track. v16 enlarges
 * the backlog to PENDING_SLOTS=84 = a full WEEK of 2 h fixes (~2 KB flash, placed above
 * the accel ring; static_asserts prove the two can never collide and both fit the
 * ~132 KB budget), and drains a big backlog on a shorter BACKLOG_GAP_SEC cadence so a
 * week replays in minutes. Newest-first + stop-at-miss + flash persistence mean a drain
 * interrupted by re-leaving range simply resumes on the next pass. Nothing else changes:
 * the transport, delivery-guarantee mechanism, BLE offload and sleep are v15-identical;
 * only the backlog DEPTH (5 -> 84), its flash location, and the drain cadence differ.
 * Flash schema bumped 3 -> 4 (one-time re-init on upgrade).
 * ----------------------------------------------------------------------------
 ...
 *      Every ACCEL_EVERY_N_FIXES-th GNSS fix we read the LIS3DHTR (v17: bit-bang SPI
 *      on the shared WuR bus) for ACCEL_SECS at ACCEL_HZ and append a fixed-size record
...
// ----------------------------------------------------------------------------
// v9/v17 - ACCELEROMETER (ONBOARD LIS3DHTR U5, bit-bang SPI on the shared WuR bus) -> FLASH RING
...
// v17 POWER (the big win of PCB iteration3): the accelerometer is now the BARE
// onboard LIS3DHTR (U5), not the Grove MODULE. The old module's ~296 uA parasitic
// (its own LED/LDO/pull-ups, measured: 335 uA floor wired vs 39 uA unplugged) is
// GONE by construction. VDD/VDD_IO are on always-on 3V3, but firmware puts the bare
// chip into power-down (CTRL1=0x00, ~0.5 uA) after each collect, so it adds ~nothing
// to the sleep floor - no switched rail needed. Expect the floor to return toward the
// v7 battery-only 34 uA (VERIFY with a clean battery-only PPK run on this board).
constexpr int      ACCEL_PWR_PIN     = -1;    // no switched accel rail on iteration3 (bare chip powers down in-place)
constexpr uint32_t ACCEL_PWR_BOOT_MS = 20;    // (unused when ACCEL_PWR_PIN < 0)
...
// ----------------------------------------------------------------------------
constexpr uint32_t TX_PULSE_GAP_SEC   = 30;   // >=30 s between the (rare) multiple FRESH packets in a pass, so one
                                              //   packet's ACK window can't collide with the next TX.
// v16: when catching up a LONG backlog after an out-of-range excursion there can be
// many pending fixes (up to PENDING_SLOTS=84). Drain them on a shorter cadence so a
// week's worth replays in minutes, not ~42 min. Still well above the ACK window
// (ACK_TIMEOUT_SEC=8 s) so windows can't collide. The gap is a DEEP-SLEEP nap, so a
// big drain costs wall-clock time, not battery. Newest-first + stop-at-miss +
// flash persistence mean a drain interrupted by re-leaving range just resumes next pass.
constexpr uint32_t BACKLOG_GAP_SEC    = 15;   // inter-packet gap while draining pending[] (backlog catch-up)
...
// ============================================================================
// ISL PIN MAP (schematic v3 / iteration3 - onboard LIS3DHTR over SPI; GPS on a
// TPS22918 load switch. RTC / GPS-UART / battery are IDENTICAL to v2.)
// ============================================================================
#define RTC_INT_PIN         P0_21
// v17 HARDWARE CHANGE #1 - GPS power switch. iteration2 used a P-FET high-side
// switch (active-LOW: LOW=ON). iteration3 uses a TPS22918 load switch whose ON
// pin is ACTIVE-HIGH (HIGH=ON, VOUT=L76K_SUPPLY). L76K_EN stays on P1.02.
// If your physical board still has the P-FET, set GPS_EN_ACTIVE_HIGH to 0.
#define GPS_EN_ACTIVE_HIGH  1         // iteration3 TPS22918: 1 = HIGH turns GPS ON
#define GPS_EN_PIN          P1_02     // L76K_EN -> TPS22918 ON
#define GPS_EN_ON_LEVEL     (GPS_EN_ACTIVE_HIGH ? HIGH : LOW)
#define GPS_EN_OFF_LEVEL    (GPS_EN_ACTIVE_HIGH ? LOW  : HIGH)
...
// v17 HARDWARE CHANGE #2 - the accelerometer is now the ONBOARD LIS3DHTR (U5) on
// a 4-wire SPI bus SHARED with the AS3933 WuR (each device has its own CS). The
// shared bus pins are the RAK4630 QSPI pins.
#define SPI_CLK_PIN         P0_03     // QSPI_CLK  (SPI_CLK)  - shared accel + WuR
#define SPI_MOSI_PIN        P0_30     // QSPI_DIO0 (SPI_MOSI) - shared
#define SPI_MISO_PIN        P0_29     // QSPI_DIO1 (SPI_MISO) - shared
#define ACCEL_CS_PIN        P0_28     // QSPI_DIO2 (SPI_CS_LIS3D) - LIS3DHTR CS, ACTIVE-LOW
#define WUR_CS_PIN          P0_26     // QSPI_CS   (SPI_CS_WuR)   - AS3933 CS, ACTIVE-HIGH -> park LOW
#define WUR_WAKE_PIN        P1_04     // AS3933 WAKE (rising on detect)
#define ACCEL_INT1_PIN      P1_03     // LIS3DH_INT1 (available; unused - collection is timer-based)
#define ACCEL_INT2_PIN      P1_01     // LIS3DH_INT2 (available; unused)
#define BATT_ADC_PIN        P0_31     // AIN7, 1M/1M divider + C17. v7: input buffer kept
                                      //   DISCONNECTED in sleep (mid-rail crowbar = ~118 uA)
#define GPS_BAUD            9600

// v17: onboard LIS3DHTR over BIT-BANG SPI (mode 3). Registers unchanged from the
// I2C part (same silicon; WHO_AM_I(0x0F) -> 0x33).
#define ACCEL_SPI_DELAY_US  4
#define LIS_ADDR            0x19     // (unused on SPI - kept for reference; SA0 tie sets I2C addr only)
#define LIS_REG_WHOAMI      0x0F
#define LIS_REG_CTRL1       0x20
#define LIS_REG_CTRL4       0x23
#define LIS_REG_OUT_X_L     0x28
#define LIS_SPI_READ        0x80     // bit7=1 read, bit6=1 auto-increment (multi-byte)
...
#define FLASH_SCHEMA_VER    4         // v16: PENDING_SLOTS 5->84 + relocated pending[] -> old flash re-inits
...
#define MAX_UNDELIVERED     8         // overflow archive (oldest real fixes rolled off pending, write-only forensic)

// v16 - DURABLE LONG-EXCURSION LoRa BACKLOG (this is the v16 feature).
// Animals routinely roam OUT of repeater range for days. GPS keeps fixing the whole
// time; the un-ACK'd real fixes are held in pending[] (flash-persisted, survives the
// drone-pass reboots) and drained newest-first the moment the link returns. v15 kept
// only PENDING_SLOTS=5 (~10 h at a 2 h cadence) and silently dropped anything older,
// so a multi-day excursion lost most of its track. v16 sizes the backlog to a full
// WEEK of 2 h fixes so the whole excursion replays on return, losing nothing until a
// >1-week outage overflows (then oldest roll to the write-only undelivered archive).
//   84 slots x 24 B = ~2 KB, placed ABOVE the accel ring; the static_asserts below
//   PROVE the accel ring and this backlog can never collide and both fit the budget.
#define PENDING_SLOTS       84        // un-ACK'd real fixes kept for retry (newest-first). 84 = 1 wk @ 2 h.
#define FLASH_OFF_PENDING   0xA000    // pending[84] = 2016 B (ends 0xA7E0), above the accel ring (ends 0x9E00)
...
// v13/v16: shared-flash budget (~132 KB usable, measured). The accel ring
// (0x0500..) and the v16 position backlog (0xA000..) share this flash, so make it a
// COMPILE-TIME invariant that (1) the accel ring can never grow into the position
// backlog, and (2) the whole map fits the budget. Grow ACCEL_RING_RECORDS or
// PENDING_SLOTS and the build fails here rather than corrupting data at runtime.
static_assert(FLASH_OFF_ACCEL_DATA + (uint32_t)ACCEL_RING_RECORDS * sizeof(AccelRecord) <= FLASH_OFF_PENDING,
              "accel ring runs into the position backlog - shrink ACCEL_RING_RECORDS or raise FLASH_OFF_PENDING");
...
// v16: the position backlog must also fit the flash budget (see the accel-ring assert).
static_assert(FLASH_OFF_PENDING + (uint32_t)PENDING_SLOTS * sizeof(PacketSlot) <= 130000UL,
              "position backlog exceeds the flash budget - reduce PENDING_SLOTS");
...
static inline void accelPinsPark();        // v17: park the shared SPI bus in sleep (defined below)
...
// v16: pending[84] is ~2 KB, past the 255 B single-call cap -> chunk it (128 B).
void savePending() { flashWriteLong(FLASH_OFF_PENDING, pending, sizeof(pending)); }
...
void flashLoadAll()
{
    ...
    if (header.magic != FLASH_MAGIC || header.version != FLASH_SCHEMA_VER) {
        ...
        saveBuffer();
        savePending();
        say("[FLASH] Fresh init (schema v4)");
        return;
    }
    flashRead(FLASH_OFF_BUFFER,  buffer,  sizeof(buffer));
    flashReadLong(FLASH_OFF_PENDING, pending, sizeof(pending));   // v16: 2 KB backlog, chunked read
...
void deepSleep(uint32_t seconds)
{
    ...
    accelPinsPark();       // v17: park the shared SPI bus (accel powered down, deselected)
...
// v17: park the shared SPI bus for sleep. The onboard LIS3DHTR is left in power-
// down (CTRL1=0x00, ~0.5 uA) and DESELECTED (CS high). We drive CS high (deselect)
// and disconnect the CLK/MOSI/MISO input buffers (PIN_CNF=2) so the MCU sources/
// sinks NO current on the bus during the long idle sleep - same "no crowbar" idea
// as the AIN7 fix. Unlike the old Grove MODULE (LED/LDO/pull-ups ~296 uA), the bare
// onboard part adds ~nothing here.
static inline void accelPinsPark()
{
    pinMode(ACCEL_CS_PIN, OUTPUT); digitalWrite(ACCEL_CS_PIN, HIGH);  // keep accel deselected
    NRF_P0->PIN_CNF[3]  = 0x00000002UL;   // P0.03 SPI_CLK  - input, buffer disconnected
    NRF_P0->PIN_CNF[30] = 0x00000002UL;   // P0.30 SPI_MOSI
    NRF_P0->PIN_CNF[29] = 0x00000002UL;   // P0.29 SPI_MISO
}
...
void gpsPowerOn()
{
    ...
    digitalWrite(GPS_EN_PIN, GPS_EN_ON_LEVEL);   // iteration3 TPS22918: HIGH = GPS ON
    delay(GPS_BOOT_DELAY_MS);
    Serial0.begin(GPS_BAUD, RAK_CUSTOM_MODE);
    say("[GPS] VCC ON (P1.02 -> TPS22918 ON, hot start via module backup battery)");
}

void gpsPowerOff()
{
    digitalWrite(GPS_EN_PIN, GPS_EN_OFF_LEVEL);                           // cut power first
...
// ============================================================================
// v17 - ONBOARD LIS3DHTR accelerometer over BIT-BANG SPI (mode 3)
//   iteration3 wires U5 to a 4-wire SPI bus shared with the AS3933 WuR (each has
//   its own CS). Bit-bang keeps this pure GPIO (no peripheral/pin-map guesswork),
//   matching how the WuR bus was already handled. SAME registers as the I2C part.
//
//   LIS3DH SPI: SPC idles HIGH; the device drives SDO on the falling edge and
//   latches SDI on the RISING edge -> CPOL=1,CPHA=1 (mode 3). Address byte:
//   bit7=RW (1=read), bit6=MS (1=auto-increment for multi-byte), bits5-0=reg.
// ============================================================================
#if ENABLE_ACCEL_BLE
static inline void spiBusBegin()
{
    pinMode(ACCEL_CS_PIN, OUTPUT); digitalWrite(ACCEL_CS_PIN, HIGH);   // deselected
    pinMode(SPI_CLK_PIN,  OUTPUT); digitalWrite(SPI_CLK_PIN,  HIGH);   // mode 3: clock idles HIGH
    pinMode(SPI_MOSI_PIN, OUTPUT); digitalWrite(SPI_MOSI_PIN, LOW);
    pinMode(SPI_MISO_PIN, INPUT);
}
// One byte, MSB first, SPI mode 3: set MOSI, drop clock (leading edge -> device
// puts SDO out), raise clock (trailing edge -> device samples SDI; we sample SDO).
static uint8_t spiXfer(uint8_t out)
{
    uint8_t in = 0;
    for (int i = 7; i >= 0; i--) {
        digitalWrite(SPI_MOSI_PIN, (out >> i) & 1);
        delayMicroseconds(ACCEL_SPI_DELAY_US);
        digitalWrite(SPI_CLK_PIN, LOW);   delayMicroseconds(ACCEL_SPI_DELAY_US);
        digitalWrite(SPI_CLK_PIN, HIGH);  delayMicroseconds(ACCEL_SPI_DELAY_US);
        in = (in << 1) | (digitalRead(SPI_MISO_PIN) & 1);
    }
    return in;
}
...
{
    digitalWrite(ACCEL_CS_PIN, LOW);
    spiXfer(reg & 0x3F);                 // RW=0 (write), MS=0
    spiXfer(val);
    digitalWrite(ACCEL_CS_PIN, HIGH);
}
static bool lisInit()
{
    spiBusBegin();
    delay(10);                           // let the part settle after power-up
    bool found = false;
    ...
    lisWriteReg(LIS_REG_CTRL4, 0x08);   // high resolution, +-2 g, 4-wire SPI (SIM=0)
    delay(10);
    return true;
}
static void lisReadXYZ(int16_t &x, int16_t &y, int16_t &z)
{
    uint8_t d[6];
    digitalWrite(ACCEL_CS_PIN, LOW);
    spiXfer(LIS_SPI_READ | LIS_SPI_AUTOINC | LIS_REG_OUT_X_L);   // 0xE8: read, auto-inc, OUT_X_L
    for (int i = 0; i < 6; i++) d[i] = spiXfer(0x00);
    digitalWrite(ACCEL_CS_PIN, HIGH);
    x = (int16_t)((d[1] << 8) | d[0]);
    y = (int16_t)((d[3] << 8) | d[2]);
    z = (int16_t)((d[5] << 8) | d[4]);
...
void collectAccelToRing(uint32_t ts)
{
    ...
    // v17 POWER: put the LIS3DH into power-down (CTRL1=0x00, ~0.5 uA vs ~11 uA at
    // 100 Hz) and park the SPI bus - so the bare onboard sensor adds ~nothing during
    // the long sleep that follows (no Grove-module parasitic on iteration3).
    lisWriteReg(LIS_REG_CTRL1, 0x00);
    if (ACCEL_PWR_PIN >= 0) digitalWrite(ACCEL_PWR_PIN, LOW);   // (no switched rail on iteration3)
    accelPinsPark();
    accelRingAppend();
}
...
void doTransmitPass()
{
    ...
    // BACKLOG_GAP_SEC apart. Stop at the first miss (link dropped): keep the rest.
    // v16: a week-long excursion can leave up to PENDING_SLOTS(84) fixes here; the
    // shorter backlog gap replays them in minutes, and the drain resumes across passes.
    if (linkUp) {
        while (header.pendingCount > 0) {
            napSleep(BACKLOG_GAP_SEC);               // backlog catch-up cadence (> ACK window)
...
{
    say("========================================================");
    say("ISL Board - PRODUCTION v17 (COLLAR: v16 ported to PCB iteration3 - ONBOARD LIS3DHTR over SPI, GPS on TPS22918)");
    say("========================================================");
    ...
    // GPS to safe idle: EN off (iteration3: LOW = off), both UART pins LOW (isolation)
    pinMode(GPS_EN_PIN, OUTPUT);      digitalWrite(GPS_EN_PIN, GPS_EN_OFF_LEVEL);
    pinMode(GPS_UART_RX_PIN, OUTPUT); digitalWrite(GPS_UART_RX_PIN, LOW);
    pinMode(GPS_UART_TX_PIN, OUTPUT); digitalWrite(GPS_UART_TX_PIN, LOW);

    // v17: shared SPI bus (accel + WuR) parked - both devices DESELECTED, bus idle.
    // Accel CS active-LOW -> park HIGH; WuR CS active-HIGH -> park LOW. Clock/MOSI
    // idle low; MISO + the two accel INT lines + WuR wake as inputs.
    pinMode(ACCEL_CS_PIN, OUTPUT); digitalWrite(ACCEL_CS_PIN, HIGH);   // deselect accel
    pinMode(WUR_CS_PIN,   OUTPUT); digitalWrite(WUR_CS_PIN,   LOW);    // deselect WuR
    pinMode(SPI_CLK_PIN,  OUTPUT); digitalWrite(SPI_CLK_PIN,  LOW);
    pinMode(SPI_MOSI_PIN, OUTPUT); digitalWrite(SPI_MOSI_PIN, LOW);
    pinMode(SPI_MISO_PIN, INPUT);
    ...
    // v17: board revision + the two iteration3 hardware interface changes.
    DBG("[CFG-BOARD] PCB iteration3  accel=ONBOARD LIS3DHTR/SPI(CS P0.28, shared WuR bus)  "
        "GPS_EN=%s(P1.02)  [RTC/UART/batt = iteration2]\r\n",
        GPS_EN_ACTIVE_HIGH ? "active-HIGH TPS22918" : "active-LOW P-FET");
    // v16: durable out-of-range backlog depth + catch-up cadence, with the coverage math.
    DBG("[CFG-BACKLOG] pending=%u fixes (~%lu h out-of-range @ %lu min) held in flash @0x%lX, "
        "drain gap=%lu s newest-first; overflow -> write-only archive\r\n",
        PENDING_SLOTS,
        (unsigned long)((uint32_t)PENDING_SLOTS * GNSS_PERIOD_MIN / 60UL),
        (unsigned long)GNSS_PERIOD_MIN, (unsigned long)FLASH_OFF_PENDING,
        (unsigned long)BACKLOG_GAP_SEC);
...
void setup()
{
    ...
    delay(300);
    say("ISL Production v17 boot. Heartbeat, then init...");
}
