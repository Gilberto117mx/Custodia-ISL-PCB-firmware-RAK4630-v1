/*
 * ISL v17 LoRa Receiver / ACK node  (reference)
 * -----------------------------------------------------------------------------
 * v17 wire format is IDENTICAL to v16/v15 (v17 is only a hardware port of the collar to
 * backlog for out-of-range excursions - nothing on the packet changed), so this
 * receiver is byte-identical to the v15 one apart from the version label. When the
 * collar catches up after being out of repeater range, you'll simply see a burst of
 * older seqs replayed newest-first, each ACK'd as normal.
 * =============================================================================
...
void setup() {
  ...
  Serial.println("=== ISL v17 LoRa Receiver (ACK node, shows GPS CELL health) ===");
  int state = sx.begin();
